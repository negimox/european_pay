...
const RoutePreviewMap = dynamic(
  () => import("@/components/trips/RoutePreviewMap"),
  { ssr: false },
);
...
export default function CreateTripModal({
    ...
    if (isOpen) {
      ...
      Promise.all([
        ...
        .then(([vData, dData]) => {
          if (vData.vehicles)
            setVehicles(
              vData.vehicles.filter((v: Vehicle) => v.status === "available"),
            );
          if (dData.drivers)
            setDrivers(
              dData.drivers.filter(
                (d: Driver) =>
                  d.status === "available" && d.license_verified === true,
              ),
            );
        })
        .catch((err) => {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!originCoords || !destinationCoords) {
      setError(
        "Please select valid Origin and Destination locations from the dropdown suggestions.",
      );
      setLoading(false);
      return;
    }
    ...
    if (selectedVehicle && cargoWeight > selectedVehicle.capacity_kg) {
      setError(
        `Cargo weight cannot exceed the selected vehicle's capacity (${selectedVehicle.capacity_kg} kg)`,
      );
      setLoading(false);
      return;
    }
          ...
          {fetching ? (
            ...
            >
                ...
                >
                  ...
                  {vehicles.map((v) => (
                    <option key={v.id} value={v.id}>
                      {v.vehicle_id} - {v.type} ({v.capacity_kg}kg)
                    </option>
                  ))}
                </select>
                {vehicles.length === 0 && (
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground">
                  Cargo Weight (kg) *{" "}
                  {maxCapacity ? `< Max: ${maxCapacity}kg` : ""}
                </label>
                <Input
                  required
                  type="number"
              ...
              <div className="space-y-2">
                ...
                <Input
                  name="notes"
                  placeholder="Optional notes for the trip... (Max 500 characters)"
                  maxLength={500}
                />
              </div>
            </form>
